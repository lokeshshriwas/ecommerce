...
const groupuery = `*[_type == "product" && (title match "*" + $searchQuery + "*" || description match "*" + $searchQuery + "*" || category->title match "*" + $searchQuery + "*" || category->slug.current match "*" + $searchQuery + "*")]`

  const products = await sanityFetch({query: groupuery, params: {searchQuery}})
  return products.data as Product[];  
}